import unittest
import tweepy #need to do this: pip install tweepy
import requests
import json
import twitter_info		# you need to initialize this file with your Tweet app info
#import sys
import unicodedata as ud
#import demjson #need to do this: pip install demjson

## SI 206 - HW
## COMMENT WITH:
## Your section day/time: Thursdays 6 PM
...
CACHE_FNAME = 'cache_twitter_terms.json' # String for your file. We want the JSON file type, bcause that way, we can easily get the information into a Python dictionary!
...
def searchTweet(s):
      ...
      results = api.search(q=s)#query
      
      #term_tweet = results["statuses"][0]
      ## Now, what are its keys?
      #print("\nThe keys of the tweet dictionary:")
      #print(term_tweet.keys())
      list_of_term_tweets = results["statuses"]#only get the first 5 tweets
      #tweetjson = demjson.encode(list_of_term_tweets)#encodes the Python object into a JSON string representation.

      tweetjson = json.dumps(list_of_term_tweets, indent=4) #encode the python list object to JSON string
      #print(type(tweetjson),"is the type of the list of term tweets")
      if s in CACHE_DICTION:
            ...
            try:
                  CACHE_DICTION[s] =  json.loads(tweetjson)
      ...
      #list_of_term_tweets = results["statuses"]

      ## Iterate over the tweets you get back...
      ## And print the text of each one!
      for tweet in list_of_term_tweets[0:5]:
            print("\n")
